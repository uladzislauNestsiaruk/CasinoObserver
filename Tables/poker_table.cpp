...
namespace {
    ...
    // Rate introduced by pair <id of combination increasing by priority, card values which characterized this combination>
    std::pair<size_t, std::vector<CardValue>> RateHand(const std::vector<Card>& table_cards, const std::vector<Card>& hand) {
        ...
        for (auto& cards : type_map) {
            ...
            for (size_t i = 0; i + 4 < cards.second.size(); ++i) {
                if (static_cast<uint8_t>(cards.second[i]) - 4 == static_cast<uint8_t>(cards.second[i + 4])) {
        ...
        for (auto& value : value_map) {
            ...
            for (size_t i = 1; i < 5; ++i) {
                no_straight |= !value_map.contains(static_cast<CardValue>(static_cast<uint8_t>(value.first) - i));
            }
        ...
        if (!straight_flash.empty()) {
            ...
            return {7, {three_of_a_kind.back(), (pairs.back() != three_of_a_kind.back() ? pairs.back() : pairs[pairs.size() - 2]) }};
        } else if (!flash.empty()) {
            return {6, flash};
    ...
    double WinProbability(size_t num_opponents, const std::vector<Card>& table_cards, const std::vector<Card>& hand) {
        std::vector<Card> all_cards(table_cards);
        all_cards.insert(all_cards.end(), hand.begin(), hand.end());
        ...
        for (size_t i = 0; i < num_iterations; ++i) {
            ...
            for (size_t j = 0; j < num_opponents; ++j) {
                std::pair<size_t, std::vector<CardValue>> opponent_rate = RateHand(exp_table_cards, {exp_deck.GetTopCard(), exp_deck.GetTopCard()});
...
PokerMoveState HumbleGambler::PokerAction(size_t num_opponents, const std::vector<Card>& table_cards, const std::vector<Card>& hand, size_t current_bet, size_t min_bet, size_t min_raise, size_t num_raises) {
    // First Move (Blinds)
    ...
    } else if ((probability < 0.3 && table_cards.size() == 5) || (probability < 0.2 && table_cards.size() == 4) ||
               (probability < 0.1 && table_cards.size() == 3) || (probability < 0.05 && table_cards.empty())) {
        ChangeGameStatus();
    ...
    if (total_bet > current_bet) {
        return {.move = (current_bet == 0 ? PokerMove::BET : PokerMove::RAISE), .money_amount = total_bet};
    }
...
void PokerTable::BettingPhase() {
    for (size_t ind = 0; bets_[ind].amount != current_bet_ || current_bet_ == 0; ind = (ind + 1) % players_.size()) {
        if (players_[ind]->GetGameStatus()) {
            PokerMoveState move_state = players_[ind]->PokerAction(players_.size(), table_cards_, players_[ind]->ShowCards(), current_bet_, min_bet_, min_raise_, bets_[ind].num_raises);
