...
WorkRoomState::WorkRoomState(StateManager* manager) {
    ...
    for (size_t ind = 0; ind < total_tables; ind++) {
        if (ind & 1) {
            tables_.push_back(std::make_shared<BlackjackClose>());
            cameras_.emplace_back(GetTextute("blackjack_dealed_state"));
        } else {
            tables_.push_back(std::make_shared<PokerClose>());
...
void WorkRoomState::HandleEvent(const sf::Event& event) {
    ...
    size_t table_ind = (event_cords.x - down_right_cords.x) / camera_size.x * grid_width +
                       (event_cords.y - down_right_cords.y) / camera_size.y;
