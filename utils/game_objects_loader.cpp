...
std::unique_ptr<GameObject> ParseGameObjects(std::string_view game_objects_path) {
    ...
    for (const auto& item : data.items()) {
        ...
        objects_graph[item.value()["parent"].template get<std::string_view>()].push_back(item.key());

        // if (!item.value().contains("parent") || !item.value().contains("coords") || !item.value().contains("type")) {
        //     throw std::logic_error(std::format("Invalid structure of {} in {}", item.key(), game_objects_path));
        // }
    }

    std::function<std::unique_ptr<GameObject>(std::string_view, const sf::Rect<int>&)> dfs = [&objects_graph, &data, &dfs](std::string_view vertex, const sf::Rect<int>& parent_sprites_rect) {
        ...
        std::array<float, 2> coords = data[vertex]["coords"].template get<std::array<float, 2>>();
        if (coords[0] < 0.0 || coords[0] > 1.0 || coords[1] > 0.0 || coords[1] < 1.0) {
            throw std::logic_error(std::format("Wrong value of \"coords\" field in {}", vertex));
        }
        sf::Vector2f pos = {parent_sprites_rect.getPosition().x + parent_sprites_rect.width * coords[0], parent_sprites_rect.getPosition().y + parent_sprites_rect.height * coords[1]};

        std::string_view type = data[vertex]["type"].template get<std::string_view>();
        if (type == "image") {
            if (!data[vertex].contains("source")) {
                throw std::logic_error(std::format("There is no \"source\" field in {}", vertex));
            }
            ...
            if (!data[vertex].contains("sources")) {
                throw std::logic_error(std::format("There is no \"sources\" field in {}", vertex));
            }

            std::vector<std::string> sources = data[vertex]["sources"].template get<std::vector<std::string>>();
